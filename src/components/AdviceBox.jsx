...
function AdviceBox({ advice, adviceNumber, handleClick, loading }) {
  return (
    <div className="flex flex-col items-center justify-center bg-dark_greyish_blue rounded-lg shadow-lg w-80 lg:w-[500px] p-5 relative">
      ...
      <div className="p-2">
        {loading ? (
          <div role="status">
            <svg
              aria-hidden="true"
              class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-neon_green"
              viewBox="0 0 100 101"
              fill="none"
      ...
      <div className="absolute bottom-[-30px] z-10">
        <button
          className="bg-neon_green rounded-full p-4 transition duration-300 ease-in-out transform hover:scale-105 lg:hover:shadow-[0_0_25px_10px_rgba(82,255,168,0.4)]"
          onClick={handleClick}
          disabled={loading}
        >
